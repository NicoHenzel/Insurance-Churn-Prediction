...
The performance of the different models will be measured by the specificity and ROC-curve. These are the most important metrics since we want to make sure that our model predicts more True negatives (= Churn) than False positives in order to give the case handlers the best overview of possible churn candidates.
...
We repeat the steps from hyperparameter
...
collect_metrics(xgb_tuned_res)

```

Then we visualize the specificity for every hyperparamater which gives us a first impression of how well specific values for our hyperparamaters perform. 
