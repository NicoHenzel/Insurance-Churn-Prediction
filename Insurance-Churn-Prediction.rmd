...
```{r display-labels-variable, echo=FALSE}
new_training_data %>% 
  count(labels) %>% 
  gt
...
```{r display-churn-variable, echo=FALSE}
df_train %>% 
  count(churn, 
        name ="churn_total") %>%
...
```{r check-preprocessing, echo=FALSE}
prepped_data <-
...
We can see that our xgboost classification algorithm performed the best out of all models so far, considering specificity and the ROC curve.
...
xgb_tuned_spec <-
  boost_tree(
      ...
      trees = 50, # number of decision trees. For testing, we can leave it at a lower value
...
```{r new-data-prediction-labels, echo=FALSE, warning=FALSE}
...
This report shows how to build a classifier algorithm for anonymized data. We have seen that our model still has room for improvement due to underfitting. I suggested, that additional feature engineering and using model ensembling could lead to better results.

The dashboard created is a mockup for displaying predictions regarding chruning to case handlers of contract data.
