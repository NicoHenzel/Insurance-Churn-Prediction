...
1. The datatype for every variable (feature column) in both datasets is double, although there are variables that only hold integer values (feature_7 to feature_15). In the **Data exploration** section we will later on see that those features truly only hold discrete values.
...
* Feature_7 - 15 appear to be discrete and feature_10 - 12 seem to be binary.
* Every other feature_ column holds continuous values.
* The scale is similar for the continuous features and the discrete features respectively. This issue will be solved in the **Data preparation** section through feature scaling (data normalization).
* Feature_1 and 2-6 have have a positive skew shown by the histograms. This will also be addressed in the **Data preparation** section by performing transformations to approximate a normal distribution for all features.
* The shown stats (mean, sd, perecentiles) are nearly impossible to interpret at this point since we don't know what the variables mean.
...
```{r correlation-matrix, fig2, out.width="90%", fig.width = 9, echo=FALSE}
...
we are interested in the correlation between `r text_spec("churn", color = "red")` and the other features:    
* It correlates not or very weak to almost all features  
...
<!-- Siehe  -->
<!-- https://www.kirenz.com/post/2021-02-17-r-classification-tidymodels/#categorical-variables -->
...
To DO:  
Variablen aus Data exploration eingrenzen und Tuning an Modell durchführen.
...
To DO
Normalisierung und feature scaling durchführen
```{r}
