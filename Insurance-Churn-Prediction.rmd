...
Any insurance company wants to improve (or at least keep) their customer loyalty.
...
2. There are `r na_training` missing values in the training and `r na_new` in the new dataset.  
...
* `r text_spec("feature_3", color = "red")` has a moderate positive correlation with `r text_spec("churn", color = "red")` (r > 0.3).  
* `r text_spec("feature_5 & 6", color = "red")` correlate slightly positive (r > 0.1).  
* `r text_spec("feature_11 & 13", color = "red")` correlate slightly negative (r < -0.1).  
* Additionally `r text_spec("feature_5 & 6", color = "red")` correlate nearly perfectly linear with each other (r ~ 1) .
...
y_var_int <-
  explore_data %>% 
  # select feature_7 - 15 since those contain int numbers
  select(8:16) %>% 
  # obtain names
  variable.names()
...
We will train our model with all available features in order to gain insights on potentially relevant features selected by the algorithm itself.
This will be done using the `r text_spec("vip()", color = "red")` function after performing the last evaluation.
...
We can see that `r text_spec("feature_5", color = "red")` has been removed. This happened by the step_corr() function in the recipe meaning feature_5 correlates strongly with another feature and is not providing additional useful information in model training.
...
Note that the value seems to be good which is a direct result of using the `r text_spec("smote()", color = "red")` function to rebalance the given data. The performance regarding specificity will change on the test data since it will still be imbalanced.
The reason we used the `r text_spec("smote()", color = "red")` function is to see which hyperparameters perform the best.
...
As expected the specificity is not as high as it was with our training data. It still improved from 0.378 to 0.415 by using hyperparamater tuning. Since it is still at a low value, our model did not predict the churn label well regarding the test data.
...
Interesting is the relevance of `r text_spec("feature_14", color = "red")` which we did not identify as relevant before.
