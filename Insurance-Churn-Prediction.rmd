...
* Am Schluss: Styling überarbeiten und Abbildungsverzeichnis erstellen  
 - Verweise in R Markdown nachschauen und Fette Textstellen als verweis markieren
...
In order to measure the performance of different classfication models, the F1-Score will serve as a metric.  
...
The performance of the different models will be measured by the specificity. This is the most important metric since we want to make sure that our model predicts more True negatives (= Churn) than False positives in order to give the case handlers the best overview of possible churn candidates.
Additionally we will check some other performance metrics like recall, precision, sensitivity, F1-Score and the ROC-AUC value to evaluate our models and hyperparameter tuning. 
...
OFFEN:  
* Plots mit GGarrange in einer Grafik zusammenfassen
Siehe:
...
TO DO:  
v = 10 setzen, wenn Auswertung finalisiert wird.
...
First we create a recipe for our model that applies the same steps to all data that we feed into our model.
...
churn_rec <-
  recipe(
    ...
    churn ~ .,
    data = train_data) %>%
  # set the features that should be update in model training but not be included in the results so we can test how they impact the performance of the model
...
We can see that `r text_spec("feature_5", color = "red")` has been removed. This happened by the step_corr() function in the recipe meaning feature_5 correlates with another feature and is not providing useful information in model training.
...
We will set up the model with the needed specification.
Normally the function `r text_spec("boost_tree", color = "red")` automatically uses the empirically best parameters if none are specified. In order to find the best parameters for our model, we will specify them. 

We also use the xgboost engine with the classification mode.
...
xgb_spec <-
  boost_tree(
      ...
      trees = 10, # number of decision trees. For testing, we will leave it at a lower value
...
Next we create a workflow object that will combine the recipe `r text_spec("churn_rec", color = "red") with our model building in **Model execution**.
...
TO DO:  
size auf 100 setzen, wenn Auswertung finalisiert wird!
...
xgb_grid <- grid_latin_hypercube(
  ...
  finalize(mtry(), train_data), # different approach since mtry depends on number of predictors in data
  learn_rate(),
  size = 20
)
...
### Explore results
First we take a look at our metrics
...
Then we visualize the ROC-AUC value for every hyperparamater which gives us a first impression of how well specific values for our hyperparamaters perform. 
...
# Pivot xgb_res to longer data format while only filtering for roc_auc
xgb_res_auc <-
  xgb_res %>%
  collect_metrics() %>%
...
# show roc-auc side by side for every hyperparameter
xgb_res_auc %>%
  ...
  labs(x = NULL,
       y = "AUC",
       title = "AUC values after tuning",
       subtitle = "For each hyperparameter")

```

### Finding the best model 

In order to train our model with the beövst hyperparameters, we evaluate them based on the ROC-AUC value depicted in the precious graph.
...
This set of values will be chosen for our hyperparameters as it overall provides the best AUC values.
...
Finally we can execute our model and fit it to our training data with the finalized workflow.  
Our validation set `r text_spec("cv_folds", color = "red")` is used to estimate the performance for specified metrics and computed by the `r text_spec("fit_resamples", color = "red")` function. In particular this is done by fitting the model to each resample while holding out a portion of the dataset from each resample on which the model is then evaluated.  
We save the predictions made so we can evaluate the performance metrics, especially the f1 score.
...
Let us execute a xgboost model without hyperparamaeter tuning in order to compare both.
...
To evaluate our models, we collect the performance metrics with `r text_spec("collect_metrics()", color = "red")`.  

First we take a look at the performance over all folds
...
This shows that the tuning did not have a great effect on our models performance since the performance metrics are nearly the same.  
The only significant change is for the specificity although it is still not great at 32 %
...
Here we can see again that the hyperparameter tuning had only a very slight effect:  
* The tuned model predicted 21796 True positives (no Churn) and 954 True negatives (Churn).
* The model with the standard settings for hyperparamaters predicted 21793 True positives (no Churn) and 922 True negatives (Churn).
...
Our model is very good at predicting that a customer won't churn (first ROC-curve) but at the same time worse than random guessing (would be a ROC-curve at 45 degrees) if a customer will churn (second ROC-curve).

For our use case we need to make sure that the model has a better specificity since we want to show the cases that are likely to churn. 
...
We can see that our hyperparameter tuned xgboost classification algorithm performed the best out of all models so far, considering specificity and F1-Score.
...
TO DO 
Specificity erhöhen, damit Model bessere Aussage treffen kann
